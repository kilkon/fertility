...
결국 지역 인구를 읽는 첫걸음은 인구를 하나의 숫자로 보지 않는 것이다. 주민등록인구는 정착의 크기를, 생활인구는 체류와 이용의 크기를, 모바일 이동량은 생활권의 흐름을 보여준다. 여기에 연령, 성별, 월별 변화를 더해야 지역이 줄어드는지, 이동하는지, 다른 방식으로 쓰이고 있는지를 구분할 수 있다.

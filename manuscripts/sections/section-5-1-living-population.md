...
다만 생활인구는 정주인구의 대체물이 아니다. 생활인구가 크다는 것은 지역이 쓰이고 있다는 뜻이지, 반드시 그 지역에서 가족이 형성되고 아이가 태어나며 학교가 유지된다는 뜻은 아니다. 그래서 생활인구는 두 방향으로 읽어야 한다. 하나는 주민등록인구만으로는 과소평가되는 행정수요를 보정하는 지표이고, 다른 하나는 그 체류와 방문을 어떻게 정주·관계·지역경제로 연결할 것인가를 묻는 정책 지표다.
...
이 차이는 지역정책의 초점을 바꾼다. 생활인구가 큰 지역에는 방문자와 체류자를 관리하는 행정역량이 필요하지만, 출산과 정주를 늘리려면 일자리, 주거, 학교, 돌봄, 의료 접근성이 함께 갖추어져야 한다. 지역이 활기 있어 보이는 것과 지역의 재생산 기반이 튼튼한 것은 다른 문제다.
...
동시에 모바일 이동량은 “사람 수”가 아니라 “이동 건수”에 가깝다는 점을 기억해야 한다. 한 사람이 여러 번 이동하면 여러 건으로 잡힐 수 있고, 이동 목적이 통근인지 관광인지 돌봄인지 소비인지는 자료만으로 바로 구분되지 않는다. 따라서 이 자료는 지역의 실제 흐름을 읽는 강력한 보조 지표이지만, 주민등록인구나 생활인구를 대체하기보다 함께 놓고 해석해야 한다. 이동이 많다는 사실은 활력의 신호일 수 있지만, 장거리 통근과 시간 빈곤의 신호일 수도 있다.
